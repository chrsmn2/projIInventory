...
<div class="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-gray-700 to-gray-800 rounded-t-xl">
        ...
        <p class="text-blue-100 text-sm mt-1">Transaction Code: <span class="font-mono font-semibold">{{ $incoming->code }}</span></p>
    </div>
    ...
    <div class="p-6 space-y-6">
        <form action="{{ route('admin.incoming.update', $incoming->id) }}" method="POST" id="incomingForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="incoming_date"
                           value="{{ old('incoming_date', $incoming->incoming_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <select id="supplier_id" name="supplier_id" class="w-full" required>
                        @if($incoming->supplier_id)
                            <option value="{{ $incoming->supplier_id }}" selected>{{ $incoming->supplier->supplier_name ?? 'Selected Supplier' }}</option>
                        @endif
                    </select>
                    <input type="hidden" id="supplier-id-value" value="{{ old('supplier_id', $incoming->supplier_id) }}">
                </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody id="itemsTableBody" class="divide-y divide-gray-200">
                            ...
                            <tr class="item-row hover:bg-gray-50">
                                <td class="px-4 py-3 text-center count text-gray-600">{{ $i + 1 }}</td>
                                <td class="px-4 py-3">
                                    <select name="items[{{ $i }}][item_id]" class="item-select w-full" required>
                                        <option value="">-- Select Item --</option>
                                        @if($itemId)
                                            <option value="{{ $itemId }}"
                                                    data-unit="{{ $unitName !== '-' ? $unitName : '' }}"
                                                    data-unit-id="{{ $unitId }}"
                                                    selected>
                                                {{ $itemName }}
                                            </option>
                                    ...
                                    <input type="hidden" class="item-id-value" value="{{ $itemId }}">
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-center unit-display text-gray-600">
                                    <span class="unit-text">{{ $unitName }}</span>
                                    <input type="hidden" name="items[{{ $i }}][unit_id]" class="unit-id-value" value="{{ $unitId }}">
                                </td>
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200">
                ...
                <a href="{{ route('admin.incoming.index') }}" class="px-6 py-2 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition">
                    Cancel
                </a>
            </div>
        </form>
...
<script>
    let itemCount = {{ is_array(old('items')) ? count(old('items')) : count($incoming->details) }};
