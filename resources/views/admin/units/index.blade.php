...
@php
    ...
    function sortUrl($column) {
        ...
        return route('admin.units.index', array_merge(request()->all(), [
            'sort_by'  => $column,
            'sort_dir' => $dir,
        ]));
    }
    ...
    <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                ...
                <thead class="bg-gray-100 border-b">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left">
                            <a href="{{ sortUrl('unit_code') }}"
                               class="inline-flex items-center gap-1 hover:text-blue-600">
                                Code <span class="text-xs">{{ sortIcon('unit_code') }}</span>
                            </a>
                        </th>
                        ...
                        <th class="px-6 py-4 text-left">
                            <a href="{{ sortUrl('unit_name') }}"
                               class="inline-flex items-center gap-1 hover:text-blue-600">
                                Name <span class="text-xs">{{ sortIcon('unit_name') }}</span>
                            </a>
                        </th>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('admin.units.edit', $unit->id) }}" class="p-1 text-blue-600 hover:text-blue-800" title="Edit">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/>
                                    </svg>
                                </a>

                                <form method="POST" action="{{ route('admin.units.destroy', $unit->id) }}" onsubmit="return confirm('Delete this unit?')" style="display:inline">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="px-6 py-4 border-t bg-gray-50 flex flex-col sm:flex-row sm:justify-between gap-4">
            <p class="text-sm text-gray-600">
                Showing
                <strong>{{ $units->firstItem() ?? 0 }}</strong>
                to
                <strong>{{ $units->lastItem() ?? 0 }}</strong>
                of
